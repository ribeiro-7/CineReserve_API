...
from .mixins.payment_mixins import PaymentMixin
from .mixins.api_error_mixins import ApiErrorMixin
...
class SeatTest(test.APITestCase, seat_mixins.SeatMixin, jwt_mixins.JWTMixin, PaymentMixin, ApiErrorMixin,):
    def setUp(self):
        cache.clear()
    ...
    def test_seat_trying_to_reserve_a_seat_thats_already_reserved_by_other_person_code_400(self):
        ...
        self.assertIn(
            ...
            self.extract_api_error(trying_to_reserve_with_different_user),
        )
    ...
    def test_seat_trying_to_reserve_a_seat_thats_already_sold_returns_code_400(self):
        ...
        seat_id = create_seat.id
        session_id = create_seat.session.id
        buying_seat = self.buy_and_complete_seats(
            session_id=session_id,
            seat_ids=[seat_id],
            access_token=admin_access_token,
        )
        self.assertEqual(buying_seat.status_code, 201)
        trying_to_reserve_sold_seat = self.reserve_seats(
            session_id=session_id,
            seat_ids=[seat_id],
        ...
        self.assertIn(
            f"Seats unavailable: {seat.row}{seat.number}",
            self.extract_api_error(trying_to_reserve_sold_seat),
        )
    ...
    def test_seat_buy_change_status_to_sold_and_code_201_ok(self):
        ...
        self.assertEqual(create_seat.status, 'Reserved')
        self.complete_payment_via_webhook(
            response.data['booking_id'],
            amount=float(response.data['amount']),
        )
        create_seat.refresh_from_db()
        self.assertEqual(create_seat.status, 'Sold')
    ...
    def test_buying_a_seat_that_has_already_been_purchased(self):
        ...
        seat_id = create_seat.id
        session_id = create_seat.session.id
        admin_buy = self.buy_and_complete_seats(
            session_id=session_id,
            seat_ids=[seat_id],
            access_token=admin_access_token,
        )
        self.assertEqual(admin_buy.status_code, 201)
        create_seat.refresh_from_db()
        self.assertEqual(create_seat.status, 'Sold')
        user_try_to_buy_same_seat = self.buy_seats(
            session_id=session_id,
            seat_ids=[seat_id],
            access_token=user_access_token,
        )
        ...
        self.assertIn(
            f"Seats unavailable: {seat.row}{seat.number}",
            self.extract_api_error(user_try_to_buy_same_seat),
        )
    ...
    def test_seat_buying_a_seat_that_has_already_been_reserved_by_other_user(self):
        ...
        self.assertIn(
            f"Seats unavailable: {seat.row}{seat.number}",
            self.extract_api_error(user_try_to_buy),
        )
    ...
    def test_celery_task_releases_seat_after_timeout(self):
        ...
        create_seat.save()
        update_seat_status_after_timeout_on_reserve(create_seat.id)
        create_seat.refresh_from_db()
    ...
    def test_celery_task_when_if_not_seat_session(self):
        ...
        with self.assertLogs(logger_name, level='INFO') as log:
            update_seat_status_after_timeout_on_reserve(seat_session_id=999)
        self.assertIn(
    ...
    def test_seat_reserve_with_invalid_seat_id_return_code_400(self):
        ...
        self.assertIn('integer', self.extract_api_error(response).lower())
    ...
    def test_seat_reserve_trying_to_reserve_a_seat_already_reserved_for_another_user(self):
        ...
        self.assertIn(
            f'Seats unavailable: {seat.row}{seat.number}',
            self.extract_api_error(response),
        )
    ...
    def test_rate_limiting_in_seat_reserve(self):
        ...
        for number in range(1, 12):
            seat = Seat.objects.create(row='F', number=number)
    ...
    def test_rate_limiting_in_seat_buy(self):
        ...
        responses = []
        for number in range(1, 12):
            create_seat = self.create_available_seat(number=str(number))
            response = self.buy_seats(
                session_id=create_seat.session.id,
                seat_ids=[create_seat.id],
                access_token=user_access_token,
            )
            responses.append(response.status_code)
        self.assertIn(429, responses)
    ...
    def test_trying_to_reserve_seat_thats_already_passed_error_400(self):
        ...
        expired_seat_id = create_expired_seat.id
        response = self.reserve_seats(
            session_id=session_id,
    ...
    def test_reserve_seat_ids_request_isnt_a_list_instance(self):
        ...
        self.assertIn('list', self.extract_api_error(response).lower())
    ...
    def test_if_reserve_seat_ids_request_is_empty(self):
        ...
        self.assertIn('at least 1', self.extract_api_error(response).lower())
    ...
    def test_if_reserve_seat_ids_request_lenght_is_different_than_seat_sessions_lenght(self):
        ...
        self.assertIn(
            'not found',
            self.extract_api_error(response).lower(),
        )
...
class SeatConcurrencyTest(test.APITransactionTestCase, jwt_mixins.JWTMixin, seat_mixins.SeatMixin, PaymentMixin,):
    def setUp(self):
        self.start_flutterwave_mocks()
